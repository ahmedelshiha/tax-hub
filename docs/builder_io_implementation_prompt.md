...
## Role & Context
You are a **senior full-stack web developer** tasked with implementing the **Admin Users Workbench Transformation**.

Your primary objective is to systematically complete all tasks outlined in the comprehensive plan:
1.  **`docs/ADMIN_USERS_WORKBENCH_TRANSFORMATION_ROADMAP.md`** (The detailed, phase-by-phase execution plan).

**Start by**:
1.  Reading the **`docs/ADMIN_WORKBENCH_QUICK_START.md`** for a high-level overview and priority matrix.
2.  Reviewing the **`docs/ADMIN_WORKBENCH_IMPLEMENTATION_SUMMARY.md`** for the project summary and key findings.
3.  Reviewing the **`docs/ADMIN_WORKBENCH_PHASE_1_5_PROGRESS.md`** for the current status and next steps.
4.  Reviewing the **`docs/ADMIN_WORKBENCH_VERIFICATION.md`** for the final verification checklist.

The project goal is to transform the current fragmented, tab-based user dashboard into a unified, high-efficiency workbench layout inspired by the Oracle Fusion pattern (Left Sidebar + Main Content + Right Insights Panel).
...
1.  **`docs/ADMIN_WORKBENCH_PHASE_1_5_PROGRESS.md`**: Update the status of the completed task and the overall phase progress.
2.  **`docs/ADMIN_USERS_WORKBENCH_TRANSFORMATION_ROADMAP.md`**: Mark the task as complete (`[x]`) and add a brief summary of the implementation details.
...
- Maintain high code reuse of existing components and hooks.
